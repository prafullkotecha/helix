...
public class ZkCache implements IZkChildListener, IZkDataListener {
	...
	public boolean set(String key, Object data) {
		...
		try {
			...
			if (zNode != null) {
				...
				_map.put(key, new ZNode(key, readData, stat));
			}
			...
			if (parentNode != null) {
				...
				Object readParentData = _client.readData(parent, parentStat);
				parentNode.setData(readParentData);
				parentNode._stat = parentStat;
			}
		} catch (ZkNoNodeException e) {
			// OK
		} finally {
	...
	public Object get(String key, Stat stat) {
		try {
			_lock.readLock().lock();
			ZNode zNode = _map.get(key);
			if (zNode == null) {
			...
			if (stat != null) {
				DataTree.copyStat(zNode._stat, stat);
			}
			// TODO: we should return a copy of data instead of ref to data in
			// cache
			return zNode._data;
		} finally {
	...
	void getKeysRecursive(String path, List<String> keys) {
		...
		if (node != null) {
			...
			for (String child : childs) {
				String childPath = path.equals("/") ? path + child : path + "/"
						+ child;
	...
	static void verify(ZkCache cache, ZkClient client, String root)
		...
		for (String key : zkMap.keySet()) {
			...
			if (actual == null) {
				if (cached != null) {
					throw new Exception(key + " not equal value. actual: "
							+ actual + ", cached: " + cached);
				}
			} else {
			...
			if (!actualChilds.equals(cachedChilds)) {
				...
				throw new Exception(key + " childs not equal. actualChilds: "
						+ actualChilds + ", cachedChilds: " + cachedChilds);
			}
		}
	}
