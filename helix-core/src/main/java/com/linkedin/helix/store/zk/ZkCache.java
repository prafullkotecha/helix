...
public class ZkCache implements IZkChildListener, IZkDataListener, IZkStateListener {
    ...
    final static ConcurrentLinkedQueue<String> changes = new ConcurrentLinkedQueue<String>();
    ...
    boolean purgeCache(String path) {
        try {
            ...
            // if a node gets purged, none of its parent nodes should be in
            // cache
            ...
            if (zNode != null) {
                ...
                // " from child set of parent:"
                // + parentPath);
                // zNode._childSet.remove(name);
                // TODO: change to LOG error
                throw new RuntimeException("Should not have parent in cache for path: " + path);
            }
    ...
    void updateCache(String path) {
        try {
            ...
            if (!_map.containsKey(path)) {
                ...
                // if we have a cache, and create timestamp is different
                // that indicates a create after delete
    ...
    public void handleDataDeleted(String dataPath) throws Exception {
        ...
        try {
            ...
            if (zNode != null) {
                ...
                // // TODO: optimize it by using cache
                // Stat stat = new Stat();
                // Object readData = _client.readData(parentPath, stat);
                // zNode.setData(readData);
                //
                // changes.add(parentPath + "-" + "handledatadeleted2-" +
                // "setcid" + stat.getCzxid());
                // zNode.setStat(stat);
            } else {
    ...
    @Override
    public void handleChildChange(String parentPath, List<String> currentChildsOnZk)
            throws Exception {
    ...
    boolean removeRecursive(String path) {
        ...
        if (zNode != null) {
            String name = path.substring(path.lastIndexOf('/') + 1);
            System.out
                    .println("Removing child:" + name + " from child set of parent:" + parentPath);
            zNode._childSet.remove(name);
        }
    ...
    public boolean remove(String key) {
        try {
            ...
            // unsubscribe from zk is done in dataDelete callback
            removeRecursive(key);
    ...
    public void updateSerialized(String path, DataUpdater<Object> updater, boolean createIfAbsent) {
        Stat stat = new Stat();
        boolean retry;
        Object oldData = null;
        ...
        while (count < 100) {
            ...
            if (!client.exists(key)) {
                ...
                if (op == 0) {
                    ...
                    // Map<String, String> toDelete = new HashMap<String,
                    // String>();
                    // read(toDelete, client, key);
                    // for (String child : toDelete.keySet()) {
    ...
    static void verify(ZkCache cache, ZkClient client, String root) throws Exception {
        ...
        System.out.println("actual size: " + zkMap.size() + ", cached size: " + cache._map.size());
