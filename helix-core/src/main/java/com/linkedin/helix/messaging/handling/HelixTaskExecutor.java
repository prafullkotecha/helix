...
//		_groupMsgHandler = new GroupMessageHandler();
	...
	public void registerMessageHandlerFactory(String type, MessageHandlerFactory factory,
		...
		if (!_handlerFactoryMap.containsKey(type)) {
			...
			LOG.info("Adding msg factory for type " + type + " threadpool size " + threadpoolSize);
		} else {
	...
	void checkResourceConfig(String resourceName, HelixManager manager) {
		if (!_resourceThreadpoolSizeMap.containsKey(resourceName)) {
			...
			if (configAccessor != null) {
				ConfigScope scope = new ConfigScopeBuilder().forCluster(manager.getClusterName())
	...
	/**
	 * Find the executor service for the message. A message can have a
	 * per-statemodelfactory executor service, or per-message type executor
	 * service.
	 * 
	 **/
...
//	public void scheduleTaskAll(List<Message> messages, List<MessageHandler> handlers,
//	        NotificationContext notificationContext) {
//		List<HelixTask> tasks = new ArrayList<HelixTask>();
//		for (int i = 0; i < messages.size(); i++)
//		{
//			Message message = messages.get(i);
//			MessageHandler handler = handlers.get(i);
//			try {
//				String taskId = message.getExeMsgId();
//
//				if (message.getMsgType().equals(MessageType.STATE_TRANSITION.toString())) {
...
//					tasks.add(task);
//				} else {
//					LOG.error("Message handling task already scheduled for " + taskId);
...
//				        "Error while executing task " + e, notificationContext.getManager()
//				                .getHelixDataAccessor());
//			}
//		}
//		
//		if (tasks.size() > 0) {
//			try {
//	            List<Future<HelixTaskResult>> futures = findExecutorServiceForMsg(messages.get(0)).invokeAll(tasks);
//	            for (int i = 0; i < messages.size(); i++) {
//					String taskId = messages.get(i).getExeMsgId();
//					LOG.info("Message:" + taskId + " handling task scheduled");
//	            	_taskMap.put(taskId, futures.get(i));
//	            }
...
//	}
	...
	public HelixTask scheduleTask(Message message, MessageHandler handler,
	        NotificationContext notificationContext) {
		HelixTask task = null;
		// assert (handler != null);
		// synchronized (_lock)
...
//				String taskId = message.getMsgId() + "/" + message.getPartitionName();
				...
				LOG.info("Scheduling message: " + taskId);
//				System.err.println("schedule message: " + taskId);
				...
				task = new HelixTask(message, notificationContext, handler, this);
				if (!_taskMap.containsKey(taskId)) {
					Future<HelixTaskResult> future = findExecutorServiceForMsg(message)
					        .submit(task);
	...
	public void cancelTask(Message message, NotificationContext notificationContext) {
		synchronized (_lock) {
			...
			if (_taskMap.containsKey(taskId)) {
				_statusUpdateUtil.logInfo(message, HelixTaskExecutor.class,
				        "Trying to cancel the future for " + taskId, notificationContext
				...
				// cancel(true)
				// is called. So state transition callbacks should implement
				// logic to
				// return
				...
				_statusUpdateUtil.logWarning(message, HelixTaskExecutor.class,
				        "Future not found when trying to cancel " + taskId, notificationContext
				                .getManager().getHelixDataAccessor());
			}
		}
	}
...
//			String taskId = message.getMsgId() + "/" + message.getPartitionName();
			...
			// if (_taskMap.containsKey(taskId))
			// {
			Future<HelixTaskResult> task = _taskMap.remove(taskId);
			// }
			// else
			if (task == null) {
	...
	public void onMessage(String instanceName, List<Message> messages,
		...
		// TODO: see if we should have a separate notification call for
		// resetting
		...
		List<String> curResourceNames = accessor.getChildNames(keyBuilder.currentStates(
		...
		for (Message message : messages) {
			// nop messages are simply removed. It is used to trigger
			// onMessage() in
			...
			if (!sessionId.equals(tgtSessionId) && !tgtSessionId.equals("*")) {
				String warningMessage = "SessionId does NOT match. expected sessionId: "
				        + sessionId + ", tgtSessionId in message: " + tgtSessionId
			...
			if (MessageState.NEW != message.getMsgState()) {
				...
				// We will read the message again if there is a new message but
				// we
			...
			// create message handlers, if handlers not found, leave its state
			// as NEW
			try {
				// List<MessageHandler> createHandlers = createMessageHandlers(message, changeContext);
...
//				handlers.addAll(createHandlers);
				...
				String error = "Failed to create message handler for " + message.getMsgId()
				        + ", exception: " + e;

				_statusUpdateUtil.logError(message, HelixStateMachineEngine.class, e, error,
				        accessor);

				// Mark message state UNPROCESSABLE if we hit an exception in
				// creating
				// message handler. The message will stay on zookeeper but will
				// not be processed
			...
			_statusUpdateUtil.logInfo(message, HelixStateMachineEngine.class, "New Message",
			        accessor);
			...
			if (!message.isControlerMsg()
				...
				if (!curResourceNames.contains(resourceName)
					...
					metaCurState.setExeBatchSize(message.getExeBatchSize());
		...
		if (readMsgs.size() > 0) {
			...
			// Map<ExecutorService, List<HelixTask>> map = new
			// HashMap<ExecutorService, List<HelixTask>>();
			for (MessageHandler handler : handlers) {
				HelixTask task = scheduleTask(handler._message, handler, changeContext);
			}
		}
	}
...
//				List<ZNRecord> list = manager
//				        .getHelixDataAccessor()
//				        .getBaseDataAccessor()
//				        .getChildren(
//				                "/" + clusterName + "/PROPERTYSTORE/PARTITION_GEN_INFO_V2/" + db,
//				                null, 0);
