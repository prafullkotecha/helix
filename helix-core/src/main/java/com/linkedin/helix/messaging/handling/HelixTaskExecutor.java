...
public class HelixTaskExecutor implements MessageListener {
	...
	if (readMsgs.size() > 0) {
		...
		// ExecutorService svc =
		// findExecutorServiceForMsg(handler._message);
		// if (!map.containsKey(svc)) {
		// map.put(svc, new ArrayList<HelixTask>());
		// }
		// map.get(svc).add(task);
	    }

	    // for (ExecutorService svc : map.keySet()) {
	    // // try {
	    // List<HelixTask> tasks = map.get(svc);
	    // // List<Future<HelixTaskResult>> futures = svc.invokeAll(tasks);
	    //
	    // for (int i = 0; i < tasks.size(); i++) {
	    // Message msg = tasks.get(i)._message;
	    // String taskId = msg.getMsgId() + "/" + msg.getPartitionName();
	    // LOG.info("Message:" + taskId + " handling task scheduled");
	    // Future<HelixTaskResult> future = svc.submit(tasks.get(i));
	    // _taskMap.put(taskId, future);
	    //
	    // }
	...
	if (!message.getGroupMessageMode()) {
	    ...
	    // subMsg.setAttribute(Attributes.PARENT_MSG_ID, message.getId());
	    //
	    // LOG.info("Creating handler for group message " +
	    // subMsg.getMsgId() + "/"
	    // + partitionName);
	    // MessageHandler handler = createMessageHandler(subMsg,
	    // changeContext);
	    // if (handler != null)
	    // {
	    // handlers.add(handler);
