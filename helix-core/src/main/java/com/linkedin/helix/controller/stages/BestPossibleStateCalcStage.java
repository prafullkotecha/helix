...
public class BestPossibleStateCalcStage extends AbstractBaseStage {
    private static final Logger logger = Logger.getLogger(BestPossibleStateCalcStage.class
	    .getName());
	...
	// System.out.println("START BestPossibleStateCalcStage.process()");
	...
	BestPossibleStateOutput bestPossibleStateOutput = compute(event, resourceMap,
	        currentStateOutput);
	event.addAttribute(AttributeName.BEST_POSSIBLE_STATE.toString(), bestPossibleStateOutput);

	long endTime = System.currentTimeMillis();
	// System.out.println("END BestPossibleStateCalcStage.process()");
	logger.info("END BestPossibleStateCalcStage.process(). took: " + (endTime - startTime)
	        + " ms");
    }
	...
	for (String resourceName : resourceMap.keySet()) {
	    ...
	    // For idealstate that has rebalancing timer and is in AUTO mode, we
	    // will run jsql
		...
		if (manager != null) {
		    queryPartitionPriorityLists = calculatePartitionPriorityListWithQuery(manager,
			    idealState);
		}
	    }

	    for (Partition partition : resource.getPartitions()) {
		Map<String, String> currentStateMap = currentStateOutput.getCurrentStateMap(
		        resourceName, partition);

		Map<String, String> bestStateForPartition;
		Set<String> disabledInstancesForPartition = cache
		        .getDisabledInstancesForPartition(partition.toString());

		if (idealState.getIdealStateMode() == IdealStateModeProperty.CUSTOMIZED) {
		    Map<String, String> idealStateMap = idealState.getInstanceStateMap(partition
			    .getPartitionName());
		    bestStateForPartition = computeCustomizedBestStateForPartition(cache,
			    stateModelDef, idealStateMap, currentStateMap,
			    disabledInstancesForPartition);
		} else if (idealState.getIdealStateMode() == IdealStateModeProperty.SEMI_CUSTOMIZED) {
		    Map<String, String> idealStateMap = idealState.getInstanceStateMap(partition
			    .getPartitionName());

		    // also counts pending state as current state and overwrite
		    // current state
		    Map<String, String> curStateMap2 = new HashMap<String, String>();
		    if (currentStateMap != null) {
			curStateMap2.putAll(currentStateMap);
		    }
		    Map<String, String> pendingStateMap = currentStateOutput.getPendingStateMap(
			    resourceName, partition);
		    if (pendingStateMap != null) {
			curStateMap2.putAll(pendingStateMap);
		    }

		    bestStateForPartition = computeSemiCustomizedBestStateForPartition(cache,
			    stateModelDef, idealStateMap, curStateMap2,
			    disabledInstancesForPartition);

		} else
		// both AUTO and AUTO_REBALANCE mode
		{
		    List<String> instancePreferenceList = getPreferenceList(cache, partition,
			...
			if (queryPartitionPriorityLists.containsKey(partitionName)) {
			    List<String> queryInstancePreferenceList = queryPartitionPriorityLists
				    .get(partitionName);
			    // For instances that is not included in the
			    // queryInstancePreferenceList,
		    ...
		    bestStateForPartition = computeAutoBestStateForPartition(cache, stateModelDef,
			    instancePreferenceList, currentStateMap, disabledInstancesForPartition);
		}
	...
	try {
	    ...
	    List<ZNRecord> resultList = JsqlQueryListProcessor.executeQueryList(
		    manager.getHelixDataAccessor(), manager.getClusterName(), queryList);
    ...
    /**
     * Compute best state for resource in AUTO_REBALANCE ideal state mode. the
     * algorithm will make sure that the master partition are evenly
     * distributed; Also when instances are added / removed, the amount of diff
     * in master partitions are minimized
     * 
     * @param cache
     * @param idealState
    ...
    private void calculateAutoBalancedIdealState(ClusterDataCache cache, IdealState idealState,
	    StateModelDefinition stateModelDef, CurrentStateOutput currentStateOutput) {
	...
	for (String liveInstanceName : liveInstances) {
	    CurrentState currentState = cache.getCurrentState(liveInstanceName,
		    cache.getLiveInstances().get(liveInstanceName).getSessionId()).get(
		    idealState.getId());
	...
	for (int i = 0; i < instanceNames.length; i++) {
	    ...
	    leave--;
	    // For hosts that has more partitions, move those partitions to
	    // "orphaned"
	...
	// Assign "orphaned" partitions to hosts that do not have enough
	// partitions
	for (int i = 0; i < instanceNames.length; i++) {
	    int targetPartitionNo = leave > 0 ? (partitionNumber + 1) : partitionNumber;
	    leave--;
		...
		masterAssignmentMap.get(instanceNames[i]).add(
		        orphanPartitions.get(lastElementIndex));
    ...
    /**
     * Generate full preference list from the master assignment map evenly
     * distribute the slave partitions mastered on a host to other hosts
     * 
     * @param masterAssignmentMap
     *            current master assignment map
     * @param orphanPartitions
     ...
     * @return
     */
    Map<String, List<String>> generateListFieldFromMasterAssignment(
	...
	if (currentStateMap != null) {
		...
		if ((instancePreferenceList == null || !instancePreferenceList.contains(instance))
		    ...
		    // if a non-error node is disabled, put it into initial
		    // state (OFFLINE)
    ...
    // HACK for ESP-0.8 cluster
    Map<String, String> computeSemiCustomizedBestStateForPartition(ClusterDataCache cache,
	    StateModelDefinition stateModelDef, Map<String, String> idealStateMap,
	    Map<String, String> currentStateMap, Set<String> disabledInstancesForPartition) {
	Map<String, String> instanceStateMap = new HashMap<String, String>();
	...
	Map<String, LiveInstance> liveInstancesMap = cache.getLiveInstances();
	int liveNb = liveInstancesMap.size();
	int disabledNb = disabledInstancesForPartition.size();
	TreeMap<String, Integer> stateCounts = getStateCounts(stateModelDef, liveNb, disabledNb,
	        liveNb);

//	System.out.println("\tstateCounts: " + stateCounts);
	final List<String> statePriorityList = stateModelDef.getStatesPriorityList();
	List<String> instanceList = new ArrayList<String>(idealStateMap.keySet());
	Collections.sort(instanceList);
	Boolean[] available = new Boolean[instanceList.size()];
	for (int i = 0; i < instanceList.size(); i++) {
	    available[i] = true;
	}
	int instanceAvailable = instanceList.size();
	
	// remove all instances that in error, not alive, or disabled
	for (int i = 0; i < instanceList.size(); i++) {
//	    if (!available[i]) {
//		continue;
//	    }
	    String instance = instanceList.get(i);
	    boolean inErrorState = currentStateMap != null
		    && "ERROR".equals(currentStateMap.get(instance));

	    if (!liveInstancesMap.containsKey(instance) || inErrorState
		    || disabledInstancesForPartition.contains(instance)) {
		available[i] = false;
		instanceAvailable--;
	    }
...
//	System.out.println("liveInstancesMap: " + liveInstancesMap);
//	System.out.println("instanceList: " + instanceList);
//	System.out.println("available: " + Arrays.asList(available));
	
	for (String state : statePriorityList) {
//	    System.out.println("state: " + state);
	    int count = stateCounts.get(state);
	    for (String instance : currentStateMap.keySet()) {
		int i = instanceList.indexOf(instance);
		if (!available[i]) {
		    continue;
		}
		String curState = currentStateMap.get(instance);
		if (curState.equals(state)) {
		    instanceStateMap.put(instance, state);
		    available[instanceList.indexOf(instance)] = false;
		    count--;
		    instanceAvailable--;
		}
	    }

	    while (count > 0 && instanceAvailable > 0) {
		for (int i = 0; i < instanceList.size(); i++) {
		    if (!available[i]) {
			continue;
		    }
		    ...
		    if (count <= 0 || instanceAvailable <= 0) {
			break;
		    }
		}
	    }
	...
	// System.out.println("\tinstanceStateMap: " + instanceStateMap);
	return instanceStateMap;
    }

    // get state and count defined by state model definition for each partition,
    // in order of state priority
    TreeMap<String, Integer> getStateCounts(StateModelDefinition stateModelDef, int liveNb,
	    int disabledNb, int preferNb) {
	final List<String> statePriorityList = stateModelDef.getStatesPriorityList();
	TreeMap<String, Integer> stateCounts = new TreeMap<String, Integer>(
	        new Comparator<String>() {

		    @Override
		    public int compare(String o1, String o2) {

		        return statePriorityList.indexOf(o1) - statePriorityList.indexOf(o2);
		    }
	        });
