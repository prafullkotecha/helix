...
public class TestHelixTaskExecutor {
	...
	public void testCMTaskExecutor() throws Exception {
		...
		HelixStateTransitionMsgHandler handler = new HelixStateTransitionMsgHandler(stateModel,
		...
		Assert.assertTrue(executor.isDone(task.getTaskId()), "task: " + task.getTaskId() + " is not done");
		Assert.assertTrue(stateModel.stateModelInvoked);
